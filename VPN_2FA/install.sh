#!/bin/bash
# installs OpenVPN on Rocky 7-8-9 Linux (with 2FA via Google Authenticator)
# Code taken from this repo: https://github.com/angristan/openvpn-install
# and adjusted specifically for Rocky 9 (using all default encryption settings)
...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
        ...
        apt install -y cmake3 git lz4 lz4-devel lzo-devel google-authenticator qrencode pam-devel pwgen
        apt -y groupinstall "Development Tools"
        apt -y copr enable dsommers/openvpn-release
        apt install -y iptables openssl wget ca-certificates curl tar 'policycoreutils-python*' openvpn
